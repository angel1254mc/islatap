...
export function parseEnvFile(text) {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    // Strip one matched pair of quotes.
...
export function parseArgs(argv) {
  ...
  for (const arg of argv) {
    if (arg === "--help" || arg === "-h") opts.help = true;
      ...
      opts.envFile = arg.slice("--env=".length);
...
export function resolveDatabaseUrl(opts, env = process.env) {
  if (opts.databaseUrl)
    return { url: opts.databaseUrl, from: "--database-url" };
  ...
  if (opts.envFile) {
    ...
    if (!parsed.DATABASE_URL)
      throw new Error(`--env: ${opts.envFile} sets no DATABASE_URL`);
...
async function rows(sql) {
  ...
  try {
    ...
    throw new Error(`Could not read the location table: ${error.message}`, {
      cause: error,
    });
  }
}
...
async function main() {
  ...
  console.log(
    `Local:   ${data.locations.length} locations, ${Object.keys(data.shapes).length} shapes`,
  );

  if (!/-pooler\./.test(target.url)) {
    console.log("");
    console.log(
      'WARNING: this does not look like a pooled Neon endpoint (no "-pooler" in the',
    );
    console.log(
      "         host). See README section 1 -- the unpooled endpoint exhausts its",
    );
  ...
  const { added, moved, changed, orphaned } = diffLocations(
    data.locations,
    before,
  );
  ...
  const fieldOnly = changed.filter(
    (c) => !moved.some((m) => m.location.id === c.location.id),
  );
  ...
  if (added.length > 0) {
    ...
    for (const l of added.slice(0, 20))
      console.log(`  ${String(l.id).padStart(4)}  ${l.name}`);
  ...
  if (orphaned.length > 0) {
    console.log(
      `In the database but not in the files (${orphaned.length}), left untouched:`,
    );
    for (const r of orphaned.slice(0, 20))
      console.log(`  ${String(r.id).padStart(4)}  ${r.name}`);
    if (orphaned.length > 20)
      console.log(`  ... and ${orphaned.length - 20} more`);
    console.log(
      "  Removing a location is manual: puzzle_round.location_id references it.",
    );
    console.log("");
  }
  ...
  if (moved.length === 0 && fieldOnly.length === 0 && added.length === 0) {
    console.log(
      "Locations are already in sync. Shapes are not diffed; run npm run db:seed if a",
    );
  ...
  if (!opts.yes) {
    if (!process.stdin.isTTY) {
      console.error(
        "Refusing to write without a TTY to confirm on. Pass --yes to proceed.",
      );
      process.exit(1);
    }
    const ok = await confirm(
      `Apply to ${describeTarget(target.url)}? Type "yes" to continue: `,
    );
    if (!ok) {
  ...
  console.log(
    `  locations ${await seedLocations(sql, data.locations)} upserted`,
  );
  ...
  const outstanding =
    residual.moved.length + residual.changed.length + residual.added.length;
